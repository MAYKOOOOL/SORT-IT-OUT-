...
public class QuitButtonTrigger : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        {
            Debug.LogError("Renderer component not found on the GameObject! Cannot swap material.");
            // Don't disable the script, as the OnMouseUpAsButton call can still work if it has a collider
        }
        else
        {
    ...
    void Start()
    {
        ...
        if (menuManager == null)
        {
            Debug.LogError("MainMenuManager not found! Cannot trigger QuitGame.");
        }
    }
    ...
    {
        Debug.Log("Quit Button: Mouse Enter (Hover)");
    ...
    {
        Debug.Log("Quit Button: Mouse Exit (Unhover)");
    ...
    {
        Debug.Log("Quit Button: Pressing Down");
    ...
    {
        Debug.Log("Quit Button: Releasing Up");
    ...
    {
        // This event only fires if OnMouseDown and OnMouseUp occurred over the same collider
        if (menuManager != null)
        {
            Debug.Log($"Mouse released over {gameObject.name}. Triggering QuitGame.");

            // Final visual cleanup before exiting
